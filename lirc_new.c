...
{int i, count=67;
	...
	int wbuf[67]={9054  ,  4444,588 ,   1647 ,    588,    1655,
              ...
              584   ,  541    , 592  ,   541  ,   591   ,  543,
              590   ,  542   ,  590   ,  542  ,   590    , 542,
              590   ,  541,     591 ,     540,     592,     548,
              584,     541,     591,     541,     591,     540,
              591,     541,     591,     541,     590,     540,
              592,     541,     591,    1650,     584,    1643,
...
{
    struct mystr *mydrv = dev_get_drvdata(dev);
    int len;
...
static ssize_t status_store(struct device *dev,
        struct device_attribute *attr, const char *newstatus, size_t val)
{int status;
    struct mystr *mydrv = dev_get_drvdata(dev);

    if (sscanf(newstatus, "%d", &status) != 1)
		return -EINVAL;
	dev_alert(dev, "changing status to %d ...\n", status);
...
static DEVICE_ATTR(hdmist, S_IRUGO | S_IWUSR, status_show,
                   status_store);

static struct attribute *mydrv_attrs[] = {
    &dev_attr_hdmist.attr,
    NULL
};
...
static int probe_ir_blaster(struct platform_device *pdev)
{
    struct mystr *str_alloc;
    int ret;

    str_alloc = devm_kzalloc(&pdev->dev, sizeof(*str_alloc), GFP_KERNEL);
    str_alloc->dev = &pdev->dev;
    platform_set_drvdata(pdev, str_alloc);
...
static int remove_ir_blaster(struct platform_device *pdev)
{
...
.probe= probe_ir_blaster,
.remove=remove_ir_blaster,

};


/*
static ssize_t lirc_rpi_driver_hdmi_show(struct device *dev,struct device_attribute *attr, char *buf)
{
...
MODULE_DESCRIPTION("module to send power off/on.");
MODULE_AUTHOR("Anon");
MODULE_LICENSE("GPL");

module_param(gpio_out_pin, int, S_IRUGO);
MODULE_PARM_DESC(gpio_out_pin, "GPIO output/transmitter pin number of the BCM"
		 " processor. (default 22");
